...
const INPUT_TYPE_LABELS: Record<string, { label: string; icon: React.ReactNode }> = {
  ...
  transcript: { label: "貼上字稿", icon: <FileText className="w-3 h-3" /> },
  link: { label: "影音連結", icon: <Link className="w-3 h-3" /> },
};
...
export function Dashboard({
  ...
  const inputMeta = INPUT_TYPE_LABELS[activeItem.inputType] ?? { label: activeItem.inputType, icon: null };
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="text-[10px] uppercase tracking-wider px-2.5 py-0.5 rounded-full bg-indigo-50 text-indigo-600 border border-indigo-100/50 dark:bg-indigo-950/40 dark:text-indigo-400 dark:border-indigo-900/30 font-bold font-mono flex items-center gap-1">
                {inputMeta.icon}
                {inputMeta.label}
              </span>
        ...
        <div className="bg-white dark:bg-zinc-950 p-12 text-center rounded-2xl border border-zinc-200 dark:border-zinc-800 flex flex-col items-center justify-center space-y-4 shadow-sm animate-pulse">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-zinc-500 max-w-sm leading-relaxed">
              AI 正在努力將本影音的核心觀點、時間軸與心智大綱完美轉換為您指定的外語。
            </p>
          </div>
        </div>
      ) : (
